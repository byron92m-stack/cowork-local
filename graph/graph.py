"""Cowork-Local: Grafo con memoria PostgreSQL."""
import logging
...
def after_executor(state: CoworkState) -> str:
    current = state.get_current_step()
    if current and current.status == "pending" and current.assigned_to == "tools":
        return "tools"
    return "reviewer"
...
def build_graph() -> StateGraph:
    ...
    workflow.add_conditional_edges("executor", after_executor, {"reviewer": "reviewer", "tools": "tools"})
