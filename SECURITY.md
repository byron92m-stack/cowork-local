...
Patient identification by cédula/RUC/passport with validation algorithm. doc_id serves as universal patient key across channels. Rate limiting: 15 msg/min Telegram, 3 emails/hr. Input sanitization: max 500 chars Telegram, 1000 chars Email, control characters stripped. Prompt injection defenses on all LLM calls. DeepSeek Flash API with JSON mode for structured outputs. Session state in Redis keyed by doc_id with 2h TTL, isolated per patient. Email queue respects Mail.ru 1/min rate limit with double protection. Patient data only accessible via authenticated doc_id. /reset unlinks telegram_chat_id without deleting records. All Redis operations async via run_in_executor. get_or_create_patient prioritizes doc_id as universal key, handles UniqueViolationError on telegram_chat_id merge.
...
Version 3.4.1 with 6 workers via sub-graph architecture + Mail.ru email + Booking Agency (Telegram) + Accounting (IMAP) + Marketing (CLI). Workers consolidated in workers/ directory. 26+ bugs fixed, production ready.
