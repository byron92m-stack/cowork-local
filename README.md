...
Independent collaborative coding assistant at ~/chat-colaborativo/. Uses deepseek/deepseek-v4-pro via Langcli (DeepSeek V4 Pro direct). Read-only consultative mode — analyzes code, reads filesystem, recommends solutions but never executes commands or modifies files. Shows code snippets for manual implementation. Launch: cd ~/chat-colaborativo && ./start.sh. Config: AGENTS.md with rules, capabilities, and project context.
