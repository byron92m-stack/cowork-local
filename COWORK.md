...
## Modes
- **Cowork**: `python apps/cli/cowork_graph.py "task"` — multi-agent, Planner(Pro) + 6 workers, 5 iterations, timeout 600s
...
- `marketing_worker` → Weekly campaign art via OpenDesign. CLI: --generate, --view, --approve, --list.
...
- Marketing: OpenDesign daemon (no cost)
...
## Marketing
CLI-based weekly campaigns: --generate (creates art via OpenDesign), --view (opens in Firefox), --approve (marks approved, sends email), --list (shows all). Templates rotate by weekday. Output: output/design/, approved: output/marketing/approved/.
...
## Infrastructure
PostgreSQL with 14 tables (8 Cowork + 6 booking + 1 accounting). Redis with _redis_safe wrapper. Graphify: 34,248 nodes, 50,881 edges. 14 MCP servers. Playwright in /browsers/. APScheduler for reminders and email queue. 88 Python files syntax-verified, 26+ bugs fixed, production ready.
